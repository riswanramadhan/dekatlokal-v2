...
function Hero({ checkupHref }: { checkupHref: string }) {
  return (
    <section aria-labelledby="hero-title" className="dl-hero-section" id="beranda">
      ...
      <div aria-hidden="true" className="dl-hero-orbit dl-hero-orbit-one" />
      <div aria-hidden="true" className="dl-hero-orbit dl-hero-orbit-two" />
      <div className="dl-hero-copy" id="konten-utama" tabIndex={-1}>
        <p className="dl-hero-eyebrow"><Sparkles aria-hidden="true" size={16} /> Ruang Tumbuh UMKM Indonesia</p>
        <h1 id="hero-title">Kenali kondisi <span className="dl-title-pill">usahamu.</span><br />Tumbuh dengan <span className="dl-title-pill is-deep">langkah tepat.</span></h1>
        <p className="dl-hero-description">Digital Checkup gratis mengubah kondisi usahamu menjadi tiga fokus, course yang relevan, dan aksi praktis yang menghasilkan Aset Usaha.</p>
        <div className="dl-hero-actions">
          <Link className="dl-primary-button" href={checkupHref}>Mulai Digital Checkup <span><ArrowRight aria-hidden="true" size={18} /></span></Link>
          <Link className="dl-secondary-button" href="/masuk">Lanjutkan hasil Checkup</Link>
        </div>
      ...
      <div className="dl-hero-visual">
        <div aria-hidden="true" className="dl-hero-blue-disc" />
        <div className="dl-owner-wrap">
          <Image alt="Pemilik UMKM Indonesia siap melanjutkan Digital Checkup" className="dl-owner-cutout" height={1050} priority sizes="(max-width: 760px) 86vw, 48vw" src="/platform-v2/hero/owner-cutout.webp" unoptimized width={820} />
        </div>
        <div className="dl-floating-card dl-floating-card-one"><span><ClipboardCheck aria-hidden="true" size={19} /></span><p><strong>8 aspek usaha</strong><small>dipetakan ringkas</small></p></div>
        <div className="dl-floating-card dl-floating-card-two"><span className="dl-focus-number">3</span><p><strong>Fokus personal</strong><small>urut sesuai dampak</small></p></div>
        <div className="dl-floating-card dl-floating-card-three"><span><LineChart aria-hidden="true" size={19} /></span><p><strong>Jejak Tumbuh</strong><small>lihat sebelum–sesudah</small></p></div>
      </div>
...
function AccessLearningSection() {
  return (
    <section aria-labelledby="access-title" className="dl-access-section dl-viewport-section" id="akses-belajar">
      <div className="dl-access-heading"><p className="dl-section-kicker">Satu jalur, dari pemetaan hingga tindakan</p><h2 id="access-title">Dari Checkup ke Course, <span className="dl-title-pill">di mana saja.</span></h2><p>Tidak perlu menebak apa yang harus dipelajari. Kondisi usahamu menentukan urutan langkah berikutnya.</p></div>
      <div className="dl-access-steps">
        {accessSteps.map((step) => {
          const Icon = step.icon;
          return (
            <article className="dl-access-card" key={step.number}>
              <div className="dl-access-media"><Image alt={step.imageAlt} fill sizes="(max-width: 699px) 90vw, (max-width: 1099px) 46vw, 24vw" src={step.image} unoptimized /><div className="dl-access-ui"><span><Icon aria-hidden="true" size={16} /></span><i><b /></i><small>{step.label}</small></div></div>
              <div className="dl-access-copy"><span>{step.number}</span><div><p>{step.label}</p><h3>{step.title}</h3><small>{step.description}</small></div></div>
            </article>
          );
        })}
      </div>
...
function FinalCta({ checkupHref }: { checkupHref: string }) {
  const collage = [
    ["/platform-v2/access/checkup.webp", "one"], ["/platform-v2/courses/branding.webp", "two"],
    ["/platform-v2/access/action.webp", "three"], ["/platform-v2/courses/keuangan.webp", "four"],
  ] as const;
  return (
    <section aria-labelledby="start-title" className="dl-final-cta dl-viewport-section" id="mulai">
      ...
      <div aria-hidden="true" className="dl-cta-collage">{collage.map(([src, position]) => <div className={`dl-collage-card is-${position}`} key={src}><Image alt="" fill sizes="260px" src={src} unoptimized /></div>)}</div>
      <div className="dl-final-copy"><p className="dl-section-kicker">Langkah pertama dimulai di sini</p><h2 id="start-title">Mulai dari satu Checkup.<br /><span className="dl-title-pill">Pulang dengan langkah jelas.</span></h2><p>Dalam beberapa menit, pahami kondisi usaha, temukan tiga fokus utama, lalu bangun Aset Usaha satu per satu.</p><div><Link className="dl-final-primary" href={checkupHref}>Mulai Digital Checkup <ArrowRight aria-hidden="true" size={18} /></Link><Link className="dl-final-secondary" href="/masuk">Lanjutkan hasil Checkup</Link></div></div>
    </section>
  );
}
